...
attack_input_param.attack_cov   = cov_attack;
...
h(1) = fimplicit(ellipse_function_handle,[-5 5 -5 5],'r','lineWidth',4);
hold on;
% Plot the DR Ellipsoid
syms xx yy
ellipse_function  = [xx yy]*DR_Ellipse*[xx;yy]-1;
ellipse_function_handle = matlabFunction(ellipse_function);
h(2) = fimplicit(ellipse_function_handle,[-5 5 -5 5],'b','lineWidth',4);
h(3) = plot(x_uniform_attack(1,:),x_uniform_attack(2,:),'.k');
grid on
hold on;
uistack(h(1), 'top');
...
ellipse_function_handle = matlabFunction(ellipse_function);
h(2) = fimplicit(ellipse_function_handle,[-5 5 -5 5],'b','lineWidth',4);
h(3) = plot(x_worst_attack(1,:),x_worst_attack(2,:),'.k');
grid on
hold on;
uistack(h(1), 'top');
xlabel('$x_1$', 'interpreter', 'latex');
