...
function attack_output_param = cvx_attack_bounding_ellipsoid(attack_input_param)
    ...
    % Compute the Attack Threshold
    output_param = compute_attack_threshold(input_param);

    % Unbox the output parameters
    attack_threshold = output_param.attack_threshold
    ...
    [min_volume,min_index] = nanmin(Ellipsoid_Volume);
    P_attack               = P_ellipse_new(:,:,min_index);
    
    % Box the output parameter
    attack_output_param.P_attack   = P_attack;
